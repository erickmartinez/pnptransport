...
def two_layers_decaying_flux(D1cms: float, Cs: float, D2cms: float, h: float,
    ...
    The code solves Poisson-Nernst-Planck equation for a single species.
    ...
    *Example*
    ...
    .. code-block:: python
        ...
        vfb, tsim, x1, c1, x2, c2, cmax = pnpfs.two_layers_decaying_flux(
            D1cms=D1,
            Cs=Cs, D2cms=D2, h=h, m=m, thickness_sinx=thickness_1, thickness_si=thickness_2,
            tempC=temp_c, voltage=voltage, time_s=time_s, na_moles=n_moles,
            h0=h0, max_surface_concentration=s,
    ...
    Cs: float
        The initial concentration of Na (1/cm\ :sup:`3`\)
    D2cms: float
    ...
    max_surface_concentration: float
        The maximum surface concentration typically observed in SiNx (1E15 cm\ :sup:`-2`\)
...
def two_layers_constant_flux(D1cms: float, D2cms: float, h: float,
    ...
    **kwargs:
    ...
    u1i = Expression(('cb', '(1-x[0]/L)*Vapp/er'), cb=Cbulk * CM3TOUM3, L=L1, Vapp=float(voltage), degree=1, er=er)
    u2i = Expression('cb', cb=Cbulk * CM3TOUM3 * m, degree=0)
...
def single_layer_zero_flux(D1cms: float, thickness_dielectric: float, tempC: float,
    ...
    **kwargs:
    ...
    M1 = xpoints_sinx
    N = tsteps
    E = voltage / thickness_dielectric / 100 / er
    dt = time_s / N
    # Estimate the diffusion coefficients for the given temperature
    tempK = tempC + 273.15
    ...
    u1i = Expression(
        ('x[0] < x1 ? cs0 : cb', '(1-x[0]/L)*Vapp/er'),
        x1=x1, cs0=cs0 * CM3TOUM3, cb=Cbulk * CM3TOUM3, L=L1, Vapp=float(voltage), er=er, degree=1
    )
