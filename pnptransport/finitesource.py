...
def two_layers_decaying_flux(D1cms: float, Cs: float, D2cms: float, h: float,
                             m: float, thickness_sinx: float,
                             thickness_si: float, tempC: float,
                             voltage: float, time_s: Union[float, int],
                             na_moles: float = 1.0, h0: float = 1E-10,
                             max_surface_concentration: float = 1E12,
    ...
    Cs: float
        The initial concentration of Na (1/cm3)
    D2cms: float
    ...
    na_moles: float
        The initial number of moles on the surface of SiNx
    h0: float
        The surface mass transfer coefficient at the top interface
    max_surface_concentration: float
        The maximum surface concentration typically observed in SiNx (1E15 cm^-2)
    ...
    **kwargs:
    ...
    E_recovery = recovery_voltage / thickness_sinx / 100 / er
    dt = time_s / N
    # Estimate the diffusion coefficients for the given temperature
    tempK = tempC + 273.15
    # Estimate the thickness of the source layer to match the corresponding
    # number of ions and the expected initial surface concentration.
    n_na = 6.022E23 * na_moles
    # Cs = (surface concentration) / thickness of the layer
    # thickness = surface concentration / Cs
    thickness_source = max_surface_concentration / Cs
    # Determine the volume of the source: Cs * V = n_na
    volume_source = n_na / Cs
    ...
    if debug:
        ...
        fcallLogger.info('moles of Na: {0:.3E} (Na atoms)'.format(na_moles))
    ...
    u1i = Expression(('cb', '(1-x[0]/L)*Vapp/er'), cb=Cbulk * CM3TOUM3, L=L1,
    ...
    def get_variational_form1(uc, up, gp1_, gp2_, u2c, cs):
        ...
        source_flux = h0ums * cs
        gc01 = -(mu1 * uc * gp1_ - source_flux)
    ...
    def update_potential_bc(uui, bias: float = voltage, s_fp: float = 0, e_traps: float = 0, ti: float = 0):
        ...
        return gp1_, gp2_, Cint_, Ctot_, E_g_, E_si_, xbar_, vfb_, s_fp, e_traps

    def update_source_concentration(c_source: float, time_: float):
        new_cs = c_source * np.exp(-h0 * time_ / thickness_source)
        new_na = new_cs * volume_source
        return new_cs, new_na
    ...
    def get_solvers_1(gp1_, gp2_, dt_, cs):

        a10 = get_variational_form1(c1_n, phi1_n, gp1_, gp2_, u2_n, cs)
        a1G = get_variational_form1(c1_G, phi1_G, gp1_, gp2_, u2_G, cs)
        a11 = get_variational_form1(c1, phi1, gp1_, gp2_, u2, cs)
    ...
    if h5fn is not None:
        ...
        with h5py.File(h5fn, 'w') as hf:
            ...
            dst.attrs['na_0'] = n_na
            dst.attrs['c_surface'] = max_surface_concentration
            dst.attrs['cs_0'] = Cs
    ...
    for n, t in enumerate(t_sim):
        ...
        gp1, gp2, Cint, Ctot, E_g, E_si, xbar, vfb[n], charge_density_fp, e_fp_traps = update_potential_bc(
            u1_n, bias=bias_t, s_fp=charge_density_fp, e_traps=e_fp_traps, ti=t
        )
        x1i, c1i, p1i = get_solution_array1(mesh1, u1_n)
        x2i, c2i = get_solution_array2(mesh2, u2_n)
        ...
        if n == (size_n - 1) or debug:
            ...
            prog_str += 'C0L = {0:.2E}, C0R={1:.2E}, C1L={2:.1E}, C1R={3:.1E}, Js1={4:.1E} '.format(cs_t, c1i[0],
                                                                                                    c1i[-1], c2i[0],
                                                                                                    js1)
            ...
            prog_str += 'Ns = {0:1.1E}'.format(n_source)

            fcallLogger.info(prog_str)

        try:
            solver1N, solver1G, f_00, f_01, f_02 = get_solvers_1(gp1, gp2, dti, cs_t * CM3TOUM3)
            ...
            gp1, gp2, _, _, _, _, _, _, _, _ = update_potential_bc(
                u1_G, bias=bias_t, s_fp=charge_density_fp, e_traps=e_fp_traps, ti=t
            )
            solver1N, solver1G, _, _, _ = get_solvers_1(gp1, gp2, dti, cs_t * CM3TOUM3)
            ...
            u2_n.assign(u2)
            # update the source
            cs_t, n_source = update_source_concentration(Cs, t)
...
def two_layers_constant_flux(D1cms: float, D2cms: float, h: float,
                             ...
                             thickness_si: float, tempC: float,
                             voltage: float, time_s: Union[float, int],
                             surface_concentration: float = 1E11, h_surface: float = 1E-4,
                             recovery_time_s: Union[float, int] = 0,
                             recovery_voltage: float = 0,
                             trapping: bool = True,
                             **kwargs):
    ...
    **kwargs:
    ...
    hums = h * 1E4  # ums/s
    flux_source = h_surface * surface_concentration
    ...
    if debug:
        ...
        fcallLogger.info('Source transport rate: {0:.3E} (1/s)'.format(h_surface))
...
def single_layers_zero_flux(D1cms: float, thickness_sinx: float, tempC: float,
                            voltage: float, time_s: Union[float, int],
                            surface_concentration: float = 1E11, zeta: float = 1E-5,
                            **kwargs):
    """
    This function simulates the flatband voltage as a function of time for a
    MIS device where Na is migrating into the cell. It also returns a matrix
    ...
    surface_concentration: float
        The initial surface concentration in atoms/cm^2
        The flux at the source will be determined as J = zeta * suface_concentration.
        Default value 1E11 atoms/cm^2
    zeta: float
    ...
    **kwargs:
    ...
    if debug:
        ...
        fcallLogger.info('Temperature: {0:.1f} °C'.format(tempC))
        fcallLogger.info('Source surface concentration: {0:.3E} (Na atoms/cm^2)'.format(surface_concentration))
        fcallLogger.info('zeta: {0:.3E} 1/s'.format(zeta))
        fcallLogger.info('Thermal voltage {0:.4} V.'.format(vth))
        fcallLogger.info('*************** SiNx ******************')
        fcallLogger.info('Thickness: {0:.1E} um'.format(thickness_sinx))
        fcallLogger.info('er: {0:.1f}'.format(er))
    ...
    flux_source = zeta * surface_concentration
    flux_source_ums = flux_source * 1E-8
    ...
    for n, t in enumerate(t_sim):
        ...
        try:
            ...
            if fcall <= max_calls:
                ...
                return single_layers_zero_flux(
                    D1cms=D1cms,
    ...
    if h5fn is not None:
        with h5py.File(h5fn, 'a') as hf:
            ...
            ds_qs[...] = np.abs(vfb * (er * e0_red) * 1E5 / q_red )
            hf['/time'].attrs['Cmax'] = c_max
            hf.close()
