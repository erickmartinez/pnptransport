...
def two_layers_source_lim(D1cms: float, Cs: float, D2cms: float, h: float,
    ...
    **kwargs:
    ...
    # In the approach of a limited source, we want to keep track of the amount
    # of ions that we are inputing with resepect to the number of ions that
    # were initial present at the source.
    # -----------------------------------------------------------------------
    ...
    if h5fn is not None:
        ...
        with h5py.File(h5fn, 'w') as hf:
            ...
            dst.attrs['na_0'] = n_na
            dst.attrs['cs_0'] = max_surface_concentration
    ...
    for n, t in enumerate(t_sim):
        ...
        if n == (size_n - 1) or debug:
            ...
            prog_str += 'C0L = {0:.4E}, C0R={1:.4E}, C1L={2:.1E}, C1R={3:.1E}, Js1={4:.1E} '.format(cs_t, c1i[0],
                                                                                                    c1i[-1], c2i[0],
                                                                                                    js1)
            prog_str += "vfb={0:.2E} V ".format(vfb[n])
            prog_str += 'D1 = {0:2.1E}, '.format(D1cms)
            prog_str += 'D2 = {0:2.1E} cm2/s '.format(D2cms)
            prog_str += 'h = {0:2.1E} cm/s '.format(h)
        ...
        try:
            solver1N, solver1G = get_solvers_1(gp1, gp2, dti, cs_t * CM3TOUM3)
