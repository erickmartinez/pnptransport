...
if __name__ == "__main__":
    ...
    n_estimators = [10, 30, 60, 100, 300, 600, 1000, 3000]  # list(np.logspace(start=-3, stop=1, num=9))

    model_rfr_mpp = RandomForestRegressor()
    model_rfr_rsh = RandomForestRegressor()

    # model_rf = RandomForestRegressor(n_estimators=1000, oob_score=True, random_state=100)
    # model_rf.fit(X_train, y_train.ravel())

    pipe_mpp = Pipeline(steps=[('normalize', StandardScaler()), ('rfr', model_rfr_mpp)])
    pipe_rsh = Pipeline(steps=[('normalize', StandardScaler()), ('rfr', model_rfr_rsh)])
    ...
    title = r"Cross Validation Curve (RFR) MPP"
    ...
    title = r"Cross Validation Curve (RFR) RSH"
    ...
    title = r"Learning Curve (RFR) estimators={0:g} MPP".format(regr_mpp.best_params_['rfr__n_estimators'])
    kf = KFold(n_splits=10, shuffle=True, random_state=10)
    plot_learning_curve(
    ...
    title = r"Learning Curve (RFR) estimators={0:g} RSH".format(regr_rsh.best_params_['rfr__n_estimators'])
    ...
    dump(
        ...
        os.path.join(results_path, 'rfr_optimized_mpp_20201118.joblib'),
        compress=1
    )

    dump(
        regr_rsh.best_estimator_,
        os.path.join(results_path, 'rfr_optimized_rsh_20201118.joblib'),
        compress=1
    )

    fig_cv_mpp.savefig(
        os.path.join(results_path, 'cross_validation_curve_mpp_rfr.png'),
        dpi=300
    )
    fig_lc_mpp.savefig(
        os.path.join(results_path, 'learning_curve_mpp_rfr.png'), dpi=300
    )
    fig_cv_rsh.savefig(
        os.path.join(results_path, 'cross_validation_rsh_rfr.png'),
        dpi=300
    )
    fig_lc_rsh.savefig(
        os.path.join(results_path, 'learning_curve_rsh_rfr.png'), dpi=300
    )

    # Save SVGs
    fig_cv_mpp.savefig(
        os.path.join(results_path, 'cross_validation_curve_mpp_rfr.svg'),
        dpi=300
    )
    fig_lc_mpp.savefig(
        os.path.join(results_path, 'learning_curve_mpp_rfr.svg'), dpi=600
    )
    fig_cv_rsh.savefig(
        os.path.join(results_path, 'cross_validation_rsh_rfr.svg'),
        dpi=600
    )
    fig_lc_rsh.savefig(
        os.path.join(results_path, 'learning_curve_rsh_rfr.svg'), dpi=600
    )
